...
document.addEventListener("DOMContentLoaded", () => {
    ...
    externalAPI.on(externalAPI.EVENT_TRACK, () => {
        ...
        if (isPlaying) {
            showNotification(track).then(_ => _).catch(_ => _);
        }
...
async function showNotification(track) {
    ...
    let coverUrl = track.cover && typeof track.cover == "string" ?
        "https://" + track.cover.replace("%%", "100x100") :
        null;
